...
/* ============================================================================
 *  TypingTitle — typewriter effect for the hero headline.
 *  ----------------------------------------------------------------------------
 *  Vanilla React (no Framer Motion needed for this) — useState drives the
 *  visible substring, useEffect ticks a setInterval that advances one character
 *  at a time with a subtle random jitter so it feels human, not robotic.
 *  After the full string lands it pauses, then erases backwards, then retypes —
 *  a natural hero loop. A blinking cursor anchors the eye.
 * ============================================================================
 */
function TypingTitle({
  text = "africa's intelligent future starts here",
  typeSpeed = 65,        // ms per character while typing
  eraseSpeed = 35,       // ms per character while erasing
  holdAfterType = 2800,  // ms to hold after full text is shown
  holdAfterErase = 500,  // ms to hold after fully erased
}) {
  const [shown, setShown] = useStateH("");
  const [phase, setPhase] = useStateH("typing"); // typing | holding | erasing | resting
  const [cursorOn, setCursorOn] = useStateH(true);

  // Blinking cursor — 530ms cadence matches the macOS terminal cursor
  useEffectH(() => {
  ...
  useEffectH(() => {
    ...
    if (phase === "typing") {
      if (shown.length < text.length) {
        // Subtle 0.7x–1.4x speed jitter for a natural rhythm
        const jitter = 0.7 + Math.random() * 0.7;
        timer = setTimeout(() => setShown(text.slice(0, shown.length + 1)), typeSpeed * jitter);
      } else {
        timer = setTimeout(() => setPhase("holding"), holdAfterType);
      }
      ...
      if (shown.length > 0) {
        ...
        timer = setTimeout(() => setPhase("resting"), holdAfterErase);
      }
    } else if (phase === "resting") {
      timer = setTimeout(() => setPhase("typing"), 50);
    }
    return () => clearTimeout(timer);
  }, [shown, phase, text, typeSpeed, eraseSpeed, holdAfterType, holdAfterErase]);

  return (
    <h1
      className="h-display neural-text-title"
      style={{ marginBottom: 24, minHeight: "1.2em", lineHeight: 1.05 }}
      aria-label={text}
    >
      <span>{shown}</span>
      <span
...
/* ============================================================================
 *  DepthBackground — Three.js depth-of-field 3D background.
 *  ----------------------------------------------------------------------------
 *  Layered translucent spheres at varied Z depths, gently drifting + parallax-
 *  responsive to the mouse. A CSS blur on the canvas itself produces the soft
 *  out-of-focus "depth of field" feel while the foreground hero text stays sharp.
 *  Runs at ~60fps on modern GPUs and gracefully degrades to a static gradient
...
function DepthBackground() {
  ...
  useEffectH(() => {
    if (typeof window === "undefined" || !window.THREE || !canvasRef.current) return;
    const THREE = window.THREE;
    const canvas = canvasRef.current;

    // Respect prefers-reduced-motion — show a static background, no animation loop
    ...
    try {
      ...
      // WebGL not available — leave the underlying CSS gradient as fallback
      console.warn("[DepthBackground] WebGL unavailable, falling back to CSS gradient.");
      return;
    }
    ...
    // Brand palette — NEXT mint, deep violet accents, warm pop
    const palette = [0x00FC8F, 0x7B5FFF, 0xFF6B9D, 0xFFC857, 0x00D4FF, 0xA67DFF];
    const layers = [];

    // 14 floating orbs spread across 3 depth bands. Closer orbs are smaller +
    // more saturated; deeper orbs are larger + softer, contributing to depth.
    for (let i = 0; i < 14; i++) {
      const band = i % 3; // 0 = close, 1 = mid, 2 = far
      const zBase = band === 0 ? -2 : band === 1 ? -6 : -11;
      const radius = band === 0 ? 0.6 + Math.random() * 0.5
                   : band === 1 ? 1.2 + Math.random() * 0.8
                   :              2.0 + Math.random() * 1.5;
      const geo = new THREE.SphereGeometry(radius, 24, 24);
      const mat = new THREE.MeshBasicMaterial({
        color: palette[i % palette.length],
        transparent: true,
        opacity: band === 0 ? 0.55 : band === 1 ? 0.35 : 0.20,
      });
    ...
    // Mouse parallax — track via normalized -1..1 coordinates
    ...
    const animate = (t) => {
      if (!running) return;
      const time = t * 0.001;
      // Ease camera toward mouse for smooth parallax (closer = more movement)
      ...
      if (!reduceMotion) {
        layers.forEach((m) => {
          const d = m.userData;
          m.position.x = d.baseX + Math.cos(time * d.speed + d.phase) * d.amp;
          m.position.y = d.baseY + Math.sin(time * d.speed * 1.3 + d.phase) * d.amp;
          m.rotation.x = time * 0.04;
          m.rotation.y = time * 0.06;
        });
      }
    ...
    const onVis = () => {
      ...
      else if (!running) { running = true; frame = requestAnimationFrame(animate); }
    };
    ...
    return () => {
      ...
      layers.forEach((m) => { m.geometry.dispose(); m.material.dispose(); });
      renderer.dispose();
    };
  }, []);
  ...
  return (
    <canvas
      ...
      style={{
        ...
        height: "100%",
        zIndex: 0,
        // The CSS blur on the canvas itself produces the "depth of field" feel
        // while text in front (zIndex 2) stays crisp.
        filter: "blur(48px) saturate(135%)",
        opacity: 0.85,
        pointerEvents: "none",
        // Fallback gradient in case WebGL fails
        background:
          "radial-gradient(ellipse at 30% 30%, rgba(123,95,255,0.25), transparent 60%)," +
          "radial-gradient(ellipse at 70% 70%, rgba(0,252,143,0.18), transparent 60%)",
      }}
    />
  );
}
  ...
  return (
    <div className="page">
      ...
      <section className="neural-hero-section" style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        {/* Three.js depth-of-field background — sits behind everything else */}
        <DepthBackground />
